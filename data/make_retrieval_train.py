...
Resilience:
  * The Vertex embedder retries RESOURCE_EXHAUSTED (429) with exponential
    backoff and checkpoints partial progress, so a mid-run quota wall pauses
    and resumes instead of throwing away everything embedded so far.
...
def _char_batches(texts, max_chars=16000, max_n=16, clip=1500):
    # Deliberately conservative: ~16K chars ~= 4-5K tokens/request, safely
    # under Vertex's ~20K per-request token ceiling. Do NOT raise these to
    # cut request count -- the tonight-error is a RATE limit (429), handled
    # by backoff + EMBED_THROTTLE_S, not a per-request size limit.
    batch, budget, out = [], 0, []
    for t in texts:
        t = str(t)[:clip]
...
class VertexEmbedder:
    ...
    def encode(self, texts, task, ckpt_path=None):
        import time
        import numpy as np
        from vertexai.language_models import TextEmbeddingInput

        texts = [str(t) for t in texts]
        throttle = float(os.environ.get("EMBED_THROTTLE_S", "0.2"))  # sec between requests

        # resume from a partial checkpoint if one exists
        out = []
        if ckpt_path and os.path.exists(ckpt_path):
            try:
                out = [list(r) for r in np.load(ckpt_path)]
                print(f"  resume: {len(out)}/{len(texts)} already embedded")
            except Exception:
                out = []

        batches = _char_batches(texts[len(out):])
        for bi, batch in enumerate(batches, 1):
            inputs = [TextEmbeddingInput(t, task) for t in batch]
            delay = 5.0
            for attempt in range(7):
                try:
                    embs = self.m.get_embeddings(inputs)
                    break
                except Exception as e:
                    msg = str(e).lower()
                    retriable = any(s in msg for s in
                                    ("resource_exhausted", "429", "exhausted", "quota"))
                    if attempt < 6 and retriable:
                        print(f"  rate limited; backoff {delay:.0f}s "
                              f"({len(out)}/{len(texts)} done)")
                        time.sleep(delay)
                        delay = min(delay * 2, 120)
                    else:
                        # persist what we have before giving up, so a re-run resumes
                        if ckpt_path and out:
                            np.save(ckpt_path, np.asarray(out, dtype="float32"))
                        raise
            out.extend([e.values for e in embs])
            if ckpt_path and bi % 10 == 0:
                np.save(ckpt_path, np.asarray(out, dtype="float32"))
            if throttle:
                time.sleep(throttle)
            if bi % 25 == 0:
                print(f"  embedded {len(out)}/{len(texts)} texts")
...
def embed_corpus(texts, embedder, cache_dir, tag, task):
    ...
    v = embedder.encode(texts, task, ckpt_path=path + ".partial")
    np.save(path, v)
    return v
